...
module.exports.put = function(req){
    ...
    return BookRequest.findById(requestId).exec().then((bookRequest)=>{
        ...
        bookRequest.status = appConfig.bookRequestStatus.returned;
        return BookRequest.update({_id:requestId}, bookRequest).exec();
    }).then(()=> {
        return Book.findById(bookId).exec();
    }).then((book)=>{
        delete book._id;
        book.status = appConfig.bookStatus.free;
        if(!book.pastRequesters){
            book.pastRequesters = [];
        }
        book.pastRequesters.push(book.requester);
        book.requester = null;
